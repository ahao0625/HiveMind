...
check("detects rm -rf", any(not x.passed for x in det.check_all("run_command", {"command": "rm -rf /"})))
check("detects traversal", any(not x.passed for x in det.check_all("read_file", {"path": "../../../etc/passwd"})))
check("allows safe", all(x.passed for x in det.check_all("read_file", {"path": "test.txt"})))
